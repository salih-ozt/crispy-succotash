...
async function initializeDatabase() {
    ...
    `);
    // 🔒 Migration: ip kolonu yoksa ekle (eski DB'ler için)
    await pool.query('ALTER TABLE post_views ADD COLUMN IF NOT EXISTS ip TEXT').catch(() => {});
...
// 🔒 rateLimit instance MUTLAKA app başlangıcında tanımlanmalı, handler içinde değil
const postCreateLimiter  = rateLimit({ windowMs: 60 * 1000, max: 30, message: { error: 'Çok fazla gönderi isteği. Lütfen bekleyin.' } });
const messageSendLimiter = rateLimit({ windowMs: 60 * 1000, max: 60, message: { error: 'Çok fazla mesaj isteği. Lütfen bekleyin.' } });

app.use('/api/posts', (req, res, next) => {
    if (req.method !== 'POST') return next();
    return postCreateLimiter(req, res, next);
});
