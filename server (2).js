...
app.use(helmet({
    contentSecurityPolicy : {
        directives: {
            defaultSrc : ["'self'"],
            scriptSrc  : ["'self'", "'unsafe-inline'", 'https://fonts.googleapis.com', 'https://cdnjs.cloudflare.com', 'https://www.google.com/recaptcha/', 'https://www.gstatic.com/recaptcha/'],
...
async function verifyRecaptcha(token, remoteip) {
    ...
    if (process.env.RECAPTCHA_ENABLED !== 'true') return { success: true, skipped: true };
    if (process.env.NODE_ENV === 'test') return { success: true, skipped: true };
    if (!process.env.RECAPTCHA_SECRET_KEY) {
        console.warn('⚠️  RECAPTCHA_SECRET_KEY tanımlı değil, doğrulama atlanıyor');
        return { success: true, skipped: true };
    }
    if (!token) return { success: false, error: 'reCAPTCHA doğrulaması gerekli' };
    try {
        const { default: fetch } = await import('node-fetch');
        ...
        if (!data.success) {
            console.warn('[reCAPTCHA] Başarısız:', data['error-codes']);
            return { success: false, error: 'reCAPTCHA doğrulaması başarısız. Lütfen tekrar deneyin.' };
        }
        return { success: true };
    } catch (e) {
        console.error('[reCAPTCHA] Doğrulama hatası:', e.message);
