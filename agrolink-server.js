...
// Şifremi Unuttum - E-posta ile şifre sıfırlama talebi (10 dakikalık token)
app.post('/api/auth/forgot-password', async (req, res) => {
    try {
        ...
        if (!user) {
            console.log(`⚠️ Şifremi unuttum talebi - Kayıtsız e-posta: ${cleanEmail}`);
            // Aynı başarılı mesajı döndür (bilgi sızdırma önleme)
            return res.json({ 
                success: true,
                message: 'Eğer bu e-posta adresi sistemimizde kayıtlıysa, şifre sıfırlama linki gönderilecektir.' 
            });
        }
